-- 0005_flashcard_sessions
--
-- Adds the minimal persistence needed for real flashcard study sessions:
--   - flashcard as an allowed study/review mode
--   - session total/rating counters
--   - review log links back to review_cards/decks
--   - learning_steps on review_cards so FSRS learning state can round-trip
...
CREATE TABLE study_sessions_v2 (
    ...
    session_type  TEXT    NOT NULL DEFAULT 'review'
                          CHECK (session_type IN ('review', 'learn', 'cram', 'flashcard')),
...
SELECT
    id, user_id, deck_id, session_type, started_at, ended_at, cards_studied,
    cards_studied, cards_correct, 0, 0, 0, 0, xp_earned, device
FROM study_sessions;
...
CREATE TABLE review_logs_v2 (
    ...
    mode               TEXT    NOT NULL DEFAULT 'review'
                               CHECK (mode IN ('review', 'learn', 'cram', 'preview', 'flashcard')),
...
SELECT
    id, user_id, word_id, NULL, NULL, session_id, rating, result,
    mode, state_before, state_after, review_duration_ms, NULL,
    device, reviewed_at
FROM review_logs;
